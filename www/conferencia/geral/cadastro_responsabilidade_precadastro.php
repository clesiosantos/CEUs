...
if(is_array($_POST['selecionados'])) {
	...
	if($_POST['selecionados'][0]){
		foreach($_POST['selecionados'] as $codigo){
			$sql = "insert into conferencia.usuarioresponsabilidade (pflcod, usucpf,  rpustatus, rpudata_inc, pcnid)
...
<td valign="top" class="title" style="border-right: 1px solid #c0c0c0; border-bottom: 1px solid #c0c0c0; border-left: 1px solid #ffffff;" colspan="3">
        <strong>Selecione o(s) Pré-cadatro(s) da(s) conferência(s)</strong>
</td>
</tr>
<tr>
<?
	  $sql = "SELECT
	  			  a.pcnid AS codigo,
				  a.pcntitulo AS descricao
				FROM
				  conferencia.preconferencia a
				WHERE
				  a.pcnstatus = 'A'
                ORDER BY
                  2";
      ...
      if(!is_array($RS)){
        ...
        for ($i=0; $i<=$nlinhas;$i++)
         ...
         ?>
                  ...
                  <tr bgcolor="<?=$cor?>">
                  <td align="right"><input type="checkbox" name="codigo" id="<?=$codigo?>" value="<?=$codigo?>" onclick="retorna(<?=$i?>);">
                                    <input type="Hidden" name="descricao" value="<?= $descricao ?>"></td>
...
<option value="">Clique no Pré-cadastro da Conferência.</option>
<?
}
?>
</select>
...
function retorna(objeto)
{
	...
	if (arCodigo[objeto].checked == true){
		...
		if (!campoSelect.options[0]){
			campoSelect.options[0] = new Option('Clique no Pré-cadastro da Conferência.', '', false, false);
		}
		sortSelect(campoSelect);
	}
}
