...
if ( $usuario->suscod == 'B' ) {
    $_SESSION['MSG_AVISO'] = array( "Sua conta está bloqueada. Para solicitar a ativação da sua conta justifique o pedido no formulário abaixo." );
...
// Validação para mandar pro cadastro de usuário do módulo Conferência.
if ($_REQUEST['sisid'] == 102) {
    header("Location: cadastrar_usuario_conferencia.php?sisid={$_REQUEST['sisid']}&usucpf={$_REQUEST['usucpf']}");
    die();
}
...
//			$tprcampo = $db->pegaUm($sql_tpr);
			foreach ( $valores as $chave => $valor) {
				$sql_proposta = "insert into
						planointerno.usuarioresponsabilidade
						( pflcod, usucpf , suoid )
					 	values
					 	( '".$pflcod."', '".$cpf."', ".$valor." )";
...
//			$emailCopia = trim( $db->pegaUm( $sqlPegaEmailSistema ) );
			...
			$assunto = "Solicitação de Cadastro no SIMINC";
			$conteudo = sprintf("%s\n<p>%s</p>\n\n%s",
			...
			//dbg($remetente);
			//dbg($emailCopia,1);
			enviar_email( $remetente, $destinatario, $assunto, $conteudo, $emailsSuperUser );
		}
...
<body>
	<div id="tutorial_theme" style="display:none"><span style="color:red;font-weight:bold;">Novidade!</span><br>Agora você pode escolher o VISUAL do seu SIMINC, clique no ícone ao lado e experimente!</div>
...
<tr>
	...
	<td align="right" style="padding-right: 30px;padding-left:10px;" >
		...
		Tema:
			<select class="select_ylw" name="theme_simec" title="Tema do SIMINC" onchange="alteraTema(this.value)" >
...
//						}
			...
			<tr>
				...
				<td><?= campo_textarea('htudsc','S','S','',100,3,''); ?><br>
				</td>
			</tr>
			<?php
			}
