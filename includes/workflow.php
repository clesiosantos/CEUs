...
/**
 * Verifica se a ação e possivel, recupera os dados da ação pelo aedid, difente
 * das outras que recuperam pelo estado de origem / destino
 * @global type $db
 * @param type $docid
 * @param type $esdiddestino
 * @param array $dados
 * @return boolean
 */
function wf_acaoPossivel3( $aedid , array $dados )
{
	global $db;

    //recupera a ação pelo id
    $acao = wf_pegarAcao2( $aedid );
...
function wf_desenhaBarraNavegacao( $docid, array $dados, $ocultar = null, $habilitarAcoes = 'S', $incluirBr = true )
{
	/*
	...
	<script type="text/javascript">
		...
		function wf_alterarEstado( aedid, docid, esdid, acao, funcaojs, parametrosjs )
		{
			...
			//logica responsável por executar a função javascript ( se existir )
			if(funcaojs != ''){
				var resposta;

				if( typeof eval(funcaojs) == 'function'){
					resposta = eval(funcaojs + parametrosjs);

					if( !resposta ){
						alert('Não é possível executar esta ação, a função da condição de javascript não foi executada com sucesso');
					    return false;
					}else{
					    if( resposta == 'stop' ){
					    	return false;
				    ...
				    alert('Não é possível executar esta ação, a função da condição de javascript não existe');
				    return false;
				}
			}
		...
		<?php if($habilitarAcoes == 'S') : ?>
    		...
    		<?php if ( count( $estados ) ) : ?>
    			...
    			<?php foreach ( $estados as $estado ) :
    						// Alteração executada porque do modo antigo se existissem ações com o mesmo
                            // destino e a mesma origem o workflow bagunçava as funções de condição.
                            //
                            // $action = wf_acaoPossivel( $docid, $estado['esdid'], $dados );
    						$action = wf_acaoPossivel3( $estado['aedid'], $dados );

    						//verificando se existe uma função setada e preparando os valores
							$funcaoJs = wf_verificaFuncaoJs($estado['aedfuncaojs'],$dados);
    			?>
					...
					<tr>
						<td style="font-size: 7pt; text-align: center; border-top: 2px solid #d0d0d0;" onmouseover="this.style.backgroundColor='#ffffdd';" onmouseout="this.style.backgroundColor='';">
							<a
								...
								onclick=" return wf_alterarEstado( '<?php echo $estado['aedid'] ?>', '<?php echo $docid ?>', '<?php echo $estado['esdid'] ?>', '<?php echo $estado['aeddscrealizar'] ?>', '<?php echo $funcaoJs['nome'] ?>', '<?php echo $funcaoJs['parametros'] ?>'  );"
    				...
    				<?php else :
    				    ...
    				    if($action === false) :
    					   if(!isset($ocultar['acaosemcondicao']) || (is_bool($ocultar['acaosemcondicao']) && !$ocultar['acaosemcondicao']) || (is_array($ocultar['acaosemcondicao']) && !in_array($estado['aedid'], $ocultar['acaosemcondicao']))) : ?>
    							<tr>
...
function wf_verificaFuncaoJs($funcao,$dados){
	if( $funcao != ''){
		$posAbre  = strpos( $funcao, "(" );
		$posFecha = strrpos( $funcao, ")" );

		if ( $posAbre === false || $posFecha === false ){
			$retorno = array('nome'         => $funcao,
					'parametros'   => '()');
		}else{
			$parametrosCru = substr( $funcao, $posAbre + 1, $posFecha - $posAbre - 1 );
			$parametrosCru = explode( ",", trim( $parametrosCru ) );
			$parametrosCru = array_map( "trim", $parametrosCru );
			$parametros    = array();
			$funcao        = trim( substr( $funcao, 0, $posAbre ) );
			...
			foreach($parametros as $chave => $dado){
				$parametros[$chave] = "\'".$dado."\'";
			}
...
function wf_pegarAcoesPorEstados( $esdid){
	global $db;
	$esdid = (integer) $esdid;
	...
	$sql = "select
			...
			inner join
				workflow.estadodocumento ed on ed.esdid = a.esdiddestino
			where
