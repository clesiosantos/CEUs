		...
		if ( $_REQUEST['formulario']) {
			...
			if ($permissao) {
				$sql = sprintf("UPDATE seguranca.usuario SET
				                ...
				                pg_escape_string($_REQUEST['usurguf']),
				                pg_escape_string($_REQUEST['usuorgexp']),
				                pg_escape_string($_REQUEST['usurgnum']),
			...
			if ( $email_aprovacao) {
				...
				$assunto = "Aprovação do Cadastro no Siminc";
				$conteudo = "
					<br/>
...
<form method="post" name="formulario">
	...
	<tr>
        ...
        <td>
            <?= campo_texto('usurgnum', 'S', 'S', '', 9, 10, '#.###.###', '', '', '', '', 'id="usurgnum"', ''); ?>
            &nbsp &nbsp
            Órgão Expeditor: <?= obrigatorio() ?>
            <?= campo_texto('usuorgexp', 'S', 'S', '', 9, 9, '', '', '', '', '', 'id="usuorgexp"', ''); ?>
            &nbsp &nbsp
            UF:
            <?php
...
<script type="text/javascript" defer="defer"><!--
	...
	function validar_formulario(){
	    ...
	    /*
		if (document.formulario.usurgnum.value == '') {
			mensagem += '\n\tNúmero de RG (Identidade)';
        }

        if (document.formulario.usuorgexp.value == '') {
        	mensagem += '\n\tÓrgão Expeditor';
        }

        if (document.formulario.usurguf.value == '') {
        	mensagem += '\n\tUF';
        }
        */
