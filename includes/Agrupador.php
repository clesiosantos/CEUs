...
<<<EOF
	<table>
		<tr>
			<td>
				<input id="busca{NOME_ORIGEM}" name="busca{NOME_ORIGEM}" class="normal" onkeydown="return movimentoComtecla{NOME_ORIGEM}(event, '', this ,'{NOME_ORIGEM}', '{NOME_DESTINO}');" onkeyup="limitaConteudoAgrupador{NOME_ORIGEM}(this,'{NOME_ORIGEM}', event,'{NOME_DESTINO}' );" onfocus="this.value = '';"  type="text" title=""  value="Pesquisar campo..." style="width: {WIDTH_SELECT}px;" />
			</td>
		</tr>
		<tr valign="middle">
			<td>
				<select id="{NOME_ORIGEM}" name="{NOME_ORIGEM}[]" multiple="multiple" size="7" style="width: {WIDTH_SELECT}px;" onkeydown="return movimentoComtecla{NOME_ORIGEM}(event, 'envia', this , '{NOME_ORIGEM}', '{NOME_DESTINO}');" onDblClick="moveSelectedOptions( document.getElementById( '{NOME_ORIGEM}' ), document.getElementById( '{NOME_DESTINO}' ), true, '' );" class="combo campoEstilo"></select>
			</td>
			<td>
...
EOF;
	...
	class Agrupador
	{
		...
		public function exibir()
		{
			...
			$html = str_replace( '{WIDTH_SELECT}', $this->width, $html );
